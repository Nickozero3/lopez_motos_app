...
if (is_post()) {
    try {
        ...
        if ($action === 'create') {
            ...
            $stock = max(0, whole_number_value(post('stock')));
            ...
            $stmt->execute([$name, nullable_text(post('sku')), nullable_text(post('category')), $stock, max(0, whole_number_value(post('min_stock'))), max(0, whole_number_value(post('buy_price'))), max(0, whole_number_value(post('sell_price'))), nullable_text(post('supplier')), $photo, $notes]);
        ...
        if ($action === 'update') {
            $partId = (int)post('id');
            $name = clean_text(post('name'));
            $newStock = max(0, whole_number_value(post('stock')));
            if ($name === '') throw new RuntimeException('El nombre del repuesto es obligatorio.');
            $photo = save_part_photo();
            $notes = sanitize_part_notes(post('notes'));
            ...
            $values = [$name, nullable_text(post('sku')), nullable_text(post('category')), $newStock, max(0, whole_number_value(post('min_stock'))), max(0, whole_number_value(post('buy_price'))), max(0, whole_number_value(post('sell_price'))), nullable_text(post('supplier')), $notes];
        ...
        if ($action === 'movement') {
            ...
            $quantity = whole_number_value(post('quantity'));
...
<section class="card span3 kpi-card"><span class="kpi-label">Unidades en stock</span><div class="kpi"><?=h(units($stats['units']))?></div><div class="kpi-foot">Suma de existencias</div></section>
...
<div class="field span4"><label>Stock actual</label><input type="number" min="0" step="1" inputmode="numeric" name="stock" value="<?=h(isset($edit['stock']) ? (int)$edit['stock'] : post('stock','0'))?>"></div><div class="field span4"><label>Stock mínimo</label><input type="number" min="0" step="1" inputmode="numeric" name="min_stock" value="<?=h(isset($edit['min_stock']) ? (int)$edit['min_stock'] : post('min_stock','0'))?>"></div><div class="field span4"><label>Precio compra</label><input type="number" min="0" step="1" inputmode="numeric" name="buy_price" value="<?=h(isset($edit['buy_price']) ? (int)$edit['buy_price'] : post('buy_price','0'))?>"></div>
<div class="field span6"><label>Precio venta</label><input type="number" min="0" step="1" inputmode="numeric" name="sell_price" value="<?=h(isset($edit['sell_price']) ? (int)$edit['sell_price'] : post('sell_price','0'))?>"></div><div class="field span6"><label>Notas</label><textarea name="notes" id="notes"><?=h($edit['notes']??post('notes'))?></textarea></div>
